...
class Ticket {
  ...
  Uint8List _encode(String text, {bool isKanji = false}) {
    ...
    return latin1.encode(text.replaceAll(nonlatin1, ''));
  }
  ...
  void row(List<PosColumn> cols) {
    ...
    for (var i = 0; i < cols.length; ++i) {
      int? colInd = 0;
      cols.sublist(0, i).forEach((col) => colInd = colInd! + col.width);
  ...
  void setModeReadOnline() {
    bytes += <int>[16, 04, 01]; // read online
    // bytes += <int>[16, 04, 04]; // out of paper query
  }
